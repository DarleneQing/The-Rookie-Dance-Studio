...
export function UsersTable({ users }: UsersTableProps) {
  ...
  // Debounced server-side search. Empty query falls back to the initial page
  // passed from the server component (bounded to 100 users).
  useEffect(() => {
    const q = search.trim()
    if (q.length < 2) {
      setResults(null)
    ...
    const timeout = setTimeout(async () => {
      try {
        const res = await searchAdminUsers(q)
        setResults(res)
      } finally {
        setSearching(false)
      }
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 w-full max-w-sm md:max-w-lg">
          ...
          <Input
            ...
            className="pl-10 bg-white/10 border-border/60 text-foreground placeholder:text-foreground/50 focus:bg-white/15"
            value={search}
              ...
              <div className="mb-4 pb-4 border-b border-white/10">
                {user.subscription ? (
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-foreground/60 font-outfit">
                  ...
                  <span className="text-foreground/60 text-sm font-outfit">No active plan</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {user.member_type === 'student' && user.verification_status === 'approved' && (
                  <RequestReVerificationDialog userId={user.id} userName={user.full_name || 'User'}>
                    <Button
                      variant="outline"
                      className="w-full h-11 bg-warning/10 hover:bg-warning/20 border-warning/30 text-warning font-outfit"
                    >
                ...
                {user.verification_status === 'reupload_required' && (
                  <div className="flex items-center justify-center gap-2 px-3 py-2 bg-warning/20 rounded-lg border border-warning/30">
                    <GraduationCap className="h-4 w-4 text-warning" />
                    <span className="text-xs text-warning font-outfit font-medium">Re-upload Required</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-white/20 hover:bg-white/10 bg-white/5">
                <TableHead className="w-[100px] text-foreground/90 font-syne font-bold px-6 py-4">Avatar</TableHead>
                <TableHead className="text-foreground/90 font-syne font-bold px-6 py-4">Name</TableHead>
                  ...
                  <TableRow key={user.id} className="border-white/20 hover:bg-white/10 transition-colors">
                    ...
                    <TableCell className="text-right px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        {user.verification_status === 'reupload_required' && (
                          <span className="text-xs text-warning font-outfit bg-warning/20 px-2 py-1 rounded-full border border-warning/30">
                            Re-upload Required
                          </span>
                        )}
                        {user.member_type === 'student' && user.verification_status === 'approved' && (
                          <RequestReVerificationDialog userId={user.id} userName={user.full_name || 'User'}>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="text-warning hover:bg-warning/20 font-outfit"
                            >
                              <RefreshCw className="mr-2 h-4 w-4" />
                              Re-verify
                            </Button>
